...
- 键盘：`W` `S` `A` `D` 自由移动，`Space` 跳跃，`Shift` 奔跑，`E` 空中冲刺，`R` 重开，`T` 房间聊天，`Tab` 打开暂停菜单
- 个人设置：鼠标灵敏度、垂直视角反转、鼠标锁定、声音和全部游戏按键自定义
- 半透明房间聊天：可靠消息通道、未读计数、发送者身份校验与 280 字符限制，消息继续遵循房主星型转发拓扑
- 本机开发者模式：在聊天中输入 `/develop` 切换，启用后按 `` ` `` 打开菜单，可实时调整生命数、飞行、无敌、移动速度和跳跃高度；参数不会影响其他玩家
- 触控：四向移动、奔跑、空中冲刺、跳跃
...
- `game/developer.ts`：本机开发者参数、默认值与安全范围归一化
